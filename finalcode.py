...
from skimage.filters import gabor, sobel, prewitt, roberts, gaussian, laplace
from skimage import exposure, measure, feature, filters
from scipy.ndimage import gaussian_laplace, distance_transform_edt
...
def calculate_surface_slope(image):
    ...
    slope = np.mean(gradient_magnitude) * (180 / np.pi)
    return slope
...
def apply_algorithms_to_images(data_dir, ground_truth_labels, num_images_to_plot=5):
    ...
    for i, image_file in enumerate(image_files):
        ...
        keypoints_sift, descriptors_sift = sift.detectAndCompute(preprocessed_image, None)
        image_with_keypoints_sift = cv2.drawKeypoints(
            image, keypoints_sift, None, flags=cv2.DrawMatchesFlags_DRAW_RICH_KEYPOINTS
        )

        keypoints_orb, descriptors_orb = orb.detectAndCompute(preprocessed_image, None)
        image_with_keypoints_orb = cv2.drawKeypoints(
            image, keypoints_orb, None, flags=cv2.DrawMatchesFlags_DRAW_RICH_KEYPOINTS
        )
        ...
        axs[0, 0].imshow(cv2.cvtColor(image_with_keypoints_sift, cv2.COLOR_BGR2RGB))
        axs[0, 0].set_title(f"SIFT Keypoints: {image_file}")
        axs[0, 0].axis('off')

        axs[0, 1].imshow(cv2.cvtColor(image_with_keypoints_orb, cv2.COLOR_BGR2RGB))
        axs[0, 1].set_title(f"ORB Keypoints: {image_file}")
        axs[0, 1].axis('off')

        axs[1, 0].imshow(edges, cmap='gray')
        axs[1, 0].set_title(f"Canny: {image_file}")
        axs[1, 0].axis('off')

        axs[1, 1].imshow(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))
        axs[1, 1].set_title(f"Safety Status: {safety_status}")
        axs[1, 1].axis('off')
        ...
        if i < len(ground_truth_labels):
            if safety_status == ground_truth_labels[i]:
...
def main():
    ...
    data_dir = st.text_input("Enter dataset directory:", "/Users/shashikumarezhil/Downloads/ai4mars-dataset-merged-0.1")
    ground_truth_input = st.text_area(
        "Enter ground truth labels (comma-separated, e.g. 'safe,unsafe,safe,unsafe'):",
        "safe,unsafe,safe,unsafe,safe,safe,unsafe,safe,unsafe,safe,safe,unsafe,safe,unsafe,safe"
    )
    ...
    if st.button("Run Safety Analysis"):
        apply_algorithms_to_images(data_dir, ground_truth_labels, num_images)
